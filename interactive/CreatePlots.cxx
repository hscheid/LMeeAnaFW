...
{
  bool saveInputDataFiles = false;
  // create plotting environment
  PlotManager plotManager;
  DefinePlotTemplates(plotManager);

  const string file_path = "../../input/mc/mcQA/";
  if (saveInputDataFiles) {
    std::vector<string> mcNames = {"LHC21b6b", "LHC21b6", "LHC19h9_fast", "LHC20f11c"};
    for (auto mc : mcNames) {
      plotManager.AddInputDataFiles(mc, {file_path + "/" + mc + ".root"});
    }

    // NB.: in order to allow running your program from everywhere on your computer it is better NOT to use relative paths
    plotManager.DumpInputDataFiles(file_path + "mcQA.XML");
  } else {
    plotManager.LoadInputDataFiles(file_path + "mcQA.XML");
  }

  { // -----------------------------------------------------------------------
    string plotName = "plot1";
    Plot plot(plotName, "test", "1d");
    // plot[1].AddData("efficiency0/Support/Pt", "LHC21b6b", "LHC21b6b").SetOptions("hist"); // path/to/hist , identifier , name
    plot[1].AddData("efficiency0/SingleElectrons/aodTrackCuts01/Nrec_Pos_eleFinalStateFromD", "LHC21b6b", "LHC21b6b").SetProjectionX();
    plot[1].AddLegend();
...
void DefinePlotTemplates(PlotManager& plotManager)
{
  ...
  { // -----------------------------------------------------------------------
    ...
    plotTemplate[0]['Y'].SetTitleOffset(1.2).SetTitleFont(43).SetTitleSize(34).SetOppositeTicks().SetMaxDigits(3).SetLog();
