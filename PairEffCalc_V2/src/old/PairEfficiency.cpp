...
double PairEfficiency::GetYield(double mass, double ptee, TH2D& cocktailPart){
  double yield = 0;
...
void PairEfficiency::ReadCocktailFile(){
  ...
  // c2.cd();
  //
  // fWeightsSum.  Draw("colz");
  // fWeightsReso. Draw("colz");
  // fWeightsCharm.Draw("colz");
...
void PairEfficiency::CalcEfficiency(){
  ...
  for (unsigned int iMCSignal = 0; iMCSignal < fMCSignalPair.size(); ++iMCSignal){
    ...
    // std::cout << "Reading generated histogram: " << histoGenName << std::endl;

    // Read histogram
    TH2D hGenInput = *((TH2D*)list_gen->FindObject(histoGenName.c_str()) ); // Read histograms from file

    if (rebin2D) fRebinner.Rebin2DHistogram(hGenInput);
    if (scale) hGenInput.Scale(1., "width");

    // Put in vector for later storing
    fGenHisto.emplace_back(hGenInput);

    // hGenInput.SetStats(false);
  ...
  for (unsigned int iMCSignal = 0; iMCSignal < fMCSignalULSLS.size(); ++iMCSignal){
    ...
    std::string histoGenName_LSpp = fPrefixGen + "LSpp_" + fMCSignalULSLS[iMCSignal];
    std::string histoGenName_LSnn = fPrefixGen + "LSnn_" + fMCSignalULSLS[iMCSignal];
    TH2D hGenInputULS  = *((TH2D*)list_gen->FindObject(histoGenName_ULS.c_str()) ); // Read histograms from file
    ...
    // Put in vector for later storing
    fGenHisto_ULS.emplace_back(hGenInputULS);

    // Create plot
    // hGenInputULS.SetStats(false);
  ...
  if (fMCSignalBaselineString.size() > 0){
    ...
    fGenHisto_ULS_Baseline.emplace_back(hGenInputULSBaseline);
  ...
  for (unsigned int iTrackCuts = 0; iTrackCuts < fCutSettings.size(); ++iTrackCuts){
    ...
    // std::cout << histFolderRecPerCut << std::endl;
    TList *list_rec = LmHelper::GetList(&fIn, histFolderRecPerCut.c_str());
    for (unsigned int iMCSignal = 0; iMCSignal < fMCSignalPair.size(); ++iMCSignal){
      std::string histoRecName = fPrefixRec + fMCSignalPair[iMCSignal];
      // std::cout << "Reading reconstructed pair-signal histogram: " << histFolderRecPerCut << "/" << histoRecName << std::endl;
      TH2D hRecInput = *((TH2D*)list_rec->FindObject(histoRecName.c_str()) ); // Read histograms from file
      ...
      fRecHisto.emplace_back(hRecInput);
    ...
    for (unsigned int iMCSignal = 0; iMCSignal < fMCSignalULSLS.size(); ++iMCSignal){
      ...
      std::string histoRecName_LSpp = fPrefixRec + "LSpp_" + fMCSignalULSLS[iMCSignal];
      std::string histoRecName_LSnn = fPrefixRec + "LSnn_" + fMCSignalULSLS[iMCSignal];

      TH2D hRecInputULS  = *((TH2D*)list_rec->FindObject(histoRecName_ULS.c_str()) ); // Read histograms from file
      ...
      // Put in vector for later storing
      fRecHisto_ULS.emplace_back(hRecInputULS);
    ...
    if (fMCSignalBaselineString.size() > 0){
      ...
      TH2D hRecInputULSBaseline  = *((TH2D*)list_rec->FindObject(histoRecName_ULS.c_str()) ); // Read histograms from file
      // also save ULS histo
      ...
      // Create plot
      // hRecInputULSBaseline.SetStats(false);
  ...
  if (vec_rec_per_MCSignal.size() > 2) std::cout << "USE ONLY ONE MCSIGNAL AND ONE MCSIGNALULSLS, for example samemother and charm" << std::endl;
  else{
    for (unsigned int iTrackCuts = 0; iTrackCuts < fCutSettings.size(); ++iTrackCuts){
      ...
      TH2D SumOverCharm = SumEff;
      SumOverCharm.SetName("SumOverCharm");
      SumOverCharm.Divide(&CharmEff);

      TH2D SumOverReso = SumEff;
      SumOverReso.SetName("SumOverReso");
      SumOverReso.Divide(&ResoEff);

      SumOverReso.SetAxisRange(0, 2, "Z");
      SumOverCharm.SetAxisRange(0, 2, "Z");
...
std::vector<TH2D> PairEfficiency::CalcEfficiencyToBaseline(const std::vector<TH2D>& effiToBeFitted, const std::vector<TH2D>& effiBaseline){
  ...
  // TF2 f2("f2", "[0]*x*x + [1]*x + [2] + [3]*y*y*y + [4]*y*y  + [5]*y + [6]", 0., 5., 0., 8.);
  TF2 f2("f2", "[0]*x*x + [1]*x + [2] + [3]*y*y + [4]*y  + [5]", 0., 5., 0., 8.);
  f2.SetParLimits(0, -0.1, 0.1);
  f2.SetParLimits(1, -0.1, 0.1);
  f2.SetParLimits(2, 0.5, 2);

  f2.SetParLimits(3, -0.1, 0.1);
  f2.SetParLimits(4, -0.1, 0.1);
  f2.SetParLimits(5, 0.5, 2);
