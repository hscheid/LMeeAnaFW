...
public:
  ...
  void SetCocktail(const LmCocktail* cockt)
  {
    fLmCocktails.clear();
    AttachCocktail(cockt);
  } // clear may crash because of missing LmCocktail destructor.
  void AttachCocktail(const LmCocktail* cockt)
  {
    fLmCocktails.push_back(new LmCocktail(*cockt));
    fCocktailProcessed = kFALSE;
  } // using copy constructor of LmCocktail.
  ...
  void SetBinsY_forprojOnX(std::vector<Double_t> bins)
  {
    fBinsPtee_forprojMee = bins;
    LmHelper::SetEmptyBinnings(fMeeBinnings, bins.size() - 1);
  } ///< Sets Y-axis bins for projecting the 2D input onto the X-axis in different slices of Y.
  void SetBinsX_forprojOnY(std::vector<Double_t> bins)
  {
    fBinsMee_forprojPtee = bins;
    LmHelper::SetEmptyBinnings(fPteeBinnings, bins.size() - 1);
  }                                                                                      ///< Sets X-axis bins for projecting the 2D input onto the Y-axis in different slices of X.
  ...
  void SetPlotRangeMee(Double_t min, Double_t max)
  {
    fPlotXMeeMin = min;
    fPlotXMeeMax = max - LmHelper::kSmallDelta;
  } // small offset to avoid bin edge for plotting.
  void SetPlotRangePtee(Double_t min, Double_t max)
  {
    fPlotXPteeMin = min;
    fPlotXPteeMax = max - LmHelper::kSmallDelta;
  }
  void SetPlotRangeY(Double_t min, Double_t max)
  {
    fPlotYMin = min;
    fPlotYMax = max;
  }
  void SetPlotRangeX(Double_t min, Double_t max)
  {
    fPlotXMin = min;
    fPlotXMax = max;
  }
  void SetPlotRangeYMargins(Double_t min, Double_t max)
  {
    fPlotYMarginBelowMin = min;
    fPlotYMarginAboveMax = max;
  } ///< Set margins used to avoid plotting through the legend/labels. Does not work with the predefined plotting functions, there please pass them directly.
  void ResetPlotRangeY()
  {
    fPlotYMin = -1;
    fPlotYMax = -1;
  }
  void ResetPlotRangeX()
  {
    fPlotXMin = -1;
    fPlotXMax = -1;
  }
  void SetPlotRangeRatio(Double_t min, Double_t max)
  {
    fPlotRatioMin = min;
    fPlotRatioMax = max;
    SetDoRatio(kTRUE);
  ...
  void SetDoRatio(bool SDR = 1, Double_t ps = 0.3, bool log = 0)
  {
    fDoRatio = SDR;
    fRatioPadSize = ps;
    fPlotRatioLogy = log;
  }                                                       ///< Activate plotting of ratio, set relative size of ratio pad, set logy.
  ...
  {
    fConfidenceLevel = CL;
    SetDoConfidenceLevel();
  }
  void SetRootfile(TDirectory* file) { fFileOut = file; }
  void SetLmSignalName(unsigned int i, std::string newname)
  {
    if (fLmSigNames.size() > i) fLmSigNames.at(i) = newname;
  } ///< Set name which *this* handler uses as legend entry or file subdirectory for signal i (does not modify the attached LmSignal). Also set by AttachSignal().
  void SetDrawStyleCocktail(const std::string st, UInt_t i = 0)
  {
    if (fLmCocktails.size() > i) fLmCocktails.at(i)->SetDrawStyle(st);
  }
  void SetDrawStyleCocktailErr(const std::string st, UInt_t i = 0)
  {
    if (fLmCocktails.size() > i) fLmCocktails.at(i)->SetDrawStyleErr(st);
  }
  void SetDivideOption(const TString st) { fDivideOption = st; }
  void SetCompareSignalsToAverage(Bool_t b = kTRUE)
  {
    fCompareSignalsToAverage = b;
    fBest[0] = fBest[1] = fBest[2] = -1;
  }                                                           ///< Activates comparison between input signals and average.
  ...
  LmCocktail* GetCocktail(UInt_t i = 0) const
  {
    if (fLmCocktails.size() > i)
      return fLmCocktails.at(i);
    else
      return 0x0;
  }
  void GetIndexBestSignals(Int_t* best)
  {
    if (fCompareSignalsToAverage) {
      best[0] = fBest[0];
      best[1] = fBest[1];
      best[2] = fBest[2];
    } else {
...
private:
  ...
  const char* GetDrawStyleCocktail(UInt_t i = 0) const
  {
    if (fLmCocktails.size() > i)
      return fLmCocktails.at(i)->GetDrawStyle();
    else
      return "hist";
  }
  const char* GetDrawStyleCocktailErr(UInt_t i = 0) const
  {
    if (fLmCocktails.size() > i)
      return fLmCocktails.at(i)->GetDrawStyleErr();
    else
      return "E2";
  }
