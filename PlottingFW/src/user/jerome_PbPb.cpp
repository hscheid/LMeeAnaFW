  ...
  if(kDoMassSlices == kFALSE){    
    ...
    if(kDoIntegrated == kTRUE){
      ...
      for(Int_t i=0; i<=n_ptee_slices; i++){
        ...
        std::cout << ptee_slices[i] << std::endl;
      }
 
      for(Int_t i=0; i<=n_mee_slices; i++)
        binsMee_forPtee.push_back(mee_slices[i]);

      std::cout << "Check 77: " << binsPtee_forMee.size() << "  " << binsMee_forPtee.size() << std::endl;
      //binsMee_forPtee.push_back(mee_bin_min);
      //binsMee_forPtee.push_back(mee_bin_max);
    }
  }
...
//    std::string photon_file = "photon_fit/input/cocktail_LF_v2.root";
  ...
  //        TH2D* own_systematic = (TH2D*)(h2_unc->Get("h2_sys_unc")->Clone("h2_sys_uncertainties"));
	...
	std::cout << "CocktailVersion:" << sconfig[ic].CocktailVersion << "  name: " << (cocktail_histograms[sconfig[ic].CocktailVersion].Data()) << std::endl; 
        ...
        if(kDoReducedCocktail){ cocktmanReso->SetInputhistAndDim(const_cast<char*>(cocktail_histograms[9+2].Data()), 2); // 2D
	...
	cocktmanReso->SetHistForNevents("NEvents");
        ...
        if (kDoBrems){
          if(!kDoMassSlices) cocktmanBrems->SetInputfiles("./input/cocktail_theory/Brems_mee_wReso.txt");
	  else            cocktmanBrems->SetInputfiles("./input/cocktail_theory/Brems_ptee.txt");
          ...
          if(!kDoMassSlices) cocktmanBrems->Process(LmCocktailMan::kFromFileMee);
	  else cocktmanBrems->Process(LmCocktailMan::kFromFilePtee);
        }
	...
	//}	
        ...
        if(kDoSUMonly || (kDoDrawOnlyFirstCock && ic>1-1)) cockt00->SetDrawOption(LmCocktail::kOnlySum);
        else if(kDoSUMnot){ 
          cockt00->SetDrawOption(LmCocktail::kAllButSum);
          cockt00->SetDrawStyleErr("hist c");
...
//          TH2D* own_systematic = (TH2D*)(h2_unc->Get("h2_sys_unc")->Clone("h2_sys_uncertainties"));
        ...
        //hanCompare->AttachSignal(*sig_combined,Form("%c%c<%c%c%c", sigVec.at(2*i)->GetName()[0], sigVec.at(2*i)->GetName()[1], sigVec.at(2*i)->GetName()[2], sigVec.at(2*i)->GetName()[3], sigVec.at(2*i)->GetName()[4]));
        hanCompare->AttachSignal(*sig_combined,Form("%s", sigVec.at(2*i)->GetName()));
        //hanCompare->AttachSignal(*sig_combined, sigVec.at(kMergeNSettings*i)->GetName());
      }
    }
    else{
...
//	hanCompare->PrintCompareRfactor_Ptee(i, printLegend); 
  ...
  if(DoCorrelationPlots == kTRUE){
    //|__________|   Plot correlations   |__________|
    std::cout << "Print Correlations!" << std::endl;
    Bool_t separatePlots=kFALSE;
    TCanvas cCorrelation; cCorrelation.SetLogx(); cCorrelation.SetLogy();
    cCorrelation.cd();
    //TH2D* haxis = new TH2D("haxis","haxis", 1000,3e-5,3e-2, 1000,1e-1,1e2);
  //  TH2D* haxis = new TH2D("haxis","haxis", 1000,1e-3,1e2, 1000,1e-5,7e-2);
        TH2D* haxis = new TH2D("haxis","haxis", 1000,1e-4,100, 1000,1e-1,10000);
    ...
    for (int i = 0; i <= hanCompare->GetMaxBinProj(projOnAxis); ++i){
	    ...
	    std::cout << "Check 88: " << hanCompare->GetMaxBinProj(projOnAxis) << std::endl;
      ...
      hanCompare->PrintCorrelation(LmSignal::kSoverB, LmSignal::kSignif, i, projOnAxis, "", kTRUE, separatePlots, &cCorrelation);
