  ...
  if (doPhiVstudy)
  {
    for(Int_t i1 = 0; i1 < n_ptee_bin; i1++){
      ...
      if (kDoPhiVCut>=LmManager::kTemplUnsub) { // if templates are needed
        ...
        hConv      = (TH3D*) LmHelper::GetHistFromTemplateFile(_fTempl, 1, "hUnlike", kTRUE);  hSig->SetName("hConv");  // conversion
        hNorm      = (TH3D*) hSig->Clone("hNorm"); hNorm->Add(hConv);
LmHelper::Info("========== templates loaded  ==========");
        hULS       = (TH3D*) LmHelper::GetHistFromTemplateFile(_fTempl, 0, "hUnlike", kTRUE);  hULS->SetName("hULS");  // prompt
        hLS        = (TH3D*) LmHelper::GetHistFromTemplateFile(_fTempl, 0, "hBkg", kTRUE);  hLS->SetName("hLS");  // prompt
...
//    std::string photon_file = "photon_fit/input/cocktail_LF_v2.root";
  ...
  if(kUseSuperConfig){
    ...
    TF1  *pol1_1 = new TF1("f1","pol1",0.2,1.2);   
    TF1  *pol2_1 = new TF1("f2","pol2",0.2,0.9);   
    TF1  *pol3_1 = new TF1("f3","pol3",0.2,1.2);   
    pol1_1->SetLineColor(kGreen+1);
    pol2_1->SetLineColor(kGreen+1);
    pol3_1->SetLineColor(kGreen+1);

    TF1  *pol1_2 = new TF1("f4","pol1",0.8,2.0);   
    TF1  *pol2_2 = new TF1("f5","pol2",0.9,2.0);   
    TF1  *pol3_2 = new TF1("f6","pol3",0.9,2.0);   
    pol1_2->SetLineColor(kPink-4);
    pol2_2->SetLineColor(kPink-4);
    pol3_2->SetLineColor(kPink-4);

    TF1 *pol4_1 = new TF1("f7", "pol4", 0.2,1.2);
    TF1 *pol8_2 = new TF1("f8", "pol5", 0.7,1.8);
    pol4_1->SetLineColor(kGreen+1);
    pol8_2->SetLineColor(kPink-4);



    TH1D* omega_yield = new TH1D("Omega_yield", "Omega raw yield", n_ptee_slices, ptee_slices );
    TH1D* phi_yield = new TH1D("Phi_yield", "Phi raw yield", n_ptee_slices, ptee_slices);

    hanCompare->Process(LmHandler::kCompare);    
    
    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,0,kTRUE)->GetDatahist()->Fit(pol4_1,"FM+","",0.6,0.85);
    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,0,kTRUE)->GetDatahist()->Fit(pol8_2,"FM+","",.8,1.4);

    TH1D* ULS_omega_0 = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,0,kTRUE)->GetDatahist()->Clone("ULS_omega_0");
    TH1D* ULS_phi_0   = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,0,kTRUE)->GetDatahist()->Clone("ULS_phi_0");

    ULS_omega_0->Add(pol4_1,-1.);
    ULS_phi_0  ->Add(pol8_2,-1.);

    TH1D* NewSubSig_0 = hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kSubSig,0,kTRUE)->GetDatahist();

    for(Int_t i = 1; i<ULS_omega_0->GetNbinsX(); i++){
      if(NewSubSig_0->GetXaxis()->GetBinCenter(i) <= 0.9){	    
        NewSubSig_0->SetBinContent(i,ULS_omega_0->GetBinContent(i));
        NewSubSig_0->SetBinError(i,  ULS_omega_0->GetBinError(i));
      }
      else {
        NewSubSig_0->SetBinContent(i,ULS_phi_0->GetBinContent(i));
        NewSubSig_0->SetBinError(i,  ULS_phi_0->GetBinError(i));
      }
    }

    TF1* omega_fit0 = new TF1("omega_fit0", "pol1(0)+crystalball(2)",0.4,2.0);
    omega_fit0->SetLineColor(kOrange);
    //omega_fit0->SetParLimits(0,0,5.00e-04);
    omega_fit0->SetParLimits(5,0,1.);
    omega_fit0->SetParLimits(6,0,9999);

    omega_fit0->SetParameter(0,5.0e-04);
    omega_fit0->SetParameter(1,-3.23243e-04);

    omega_fit0->SetParameter(2,0.001);
    omega_fit0->SetParameter(3,0.78);
    omega_fit0->SetParameter(4,0.01);
    omega_fit0->SetParameter(5,0.1);
    omega_fit0->SetParameter(6,0.5);
    omega_fit0->SetNpx(1000);

    TF1* phi_fit0 = new TF1("phi_fit0", "pol1(0)+crystalball(2)",0.88,1.5);
    //phi_fit0->SetParLimits(0,0,6.00e-04);
    phi_fit0->SetParLimits(5,0,1.);
    phi_fit0->SetParLimits(6,0,9999);
    phi_fit0->SetParameter(0,6.0e-04);
    phi_fit0->SetParameter(1,-3.23243e-04);
    phi_fit0->SetNpx(1000);


    phi_fit0->SetParameter(2,0.0001);
    phi_fit0->SetParameter(3,1.05);
    phi_fit0->SetParameter(4,0.01);
    phi_fit0->SetParameter(5,0.01);
    phi_fit0->SetParameter(6,0.5);

    NewSubSig_0->Fit(omega_fit0,"MR+","",0.65,1.3);
    NewSubSig_0->Fit(phi_fit0,"MR+","",0.9,1.4);

    TH1D* Sig_0 = (TH1D*)NewSubSig_0->Clone("Sig0");
    TF1* sig_0_bkg = new TF1("sig_0_bkg", "pol1", 0.4,2.0);
    Double_t par0[2];
    omega_fit0->GetParameters(&par0[0]);
    sig_0_bkg->SetParameters(par0);
    Sig_0->Add(sig_0_bkg,-1.);
    Double_t omega_0_yield, omega_0_error;
    Double_t phi_0_yield, phi_0_error;

    omega_0_yield = Sig_0->IntegralAndError(Sig_0->GetXaxis()->FindBin(0.65), Sig_0->GetXaxis()->FindBin(0.85), omega_0_error);
    phi_0_yield = Sig_0->IntegralAndError(Sig_0->GetXaxis()->FindBin(0.95), Sig_0->GetXaxis()->FindBin(1.1), phi_0_error);

    omega_yield->SetBinContent(1,omega_0_yield);
    omega_yield->SetBinError(1,omega_0_error);

    phi_yield->SetBinContent(1,phi_0_yield);
    phi_yield->SetBinError(1,phi_0_error);

    Double_t SoverB_omega_0 = omega_0_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,0,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.6), Sig_0->GetXaxis()->FindBin(0.85));
    Double_t SoverB_phi_0   = phi_0_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,0,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.85), Sig_0->GetXaxis()->FindBin(1.1));

    std::cout << "S/B: omega: " << SoverB_omega_0 << "  phi: " << SoverB_phi_0 << std::endl;
    std::cout << "0. Bin Omega Integral: " << omega_0_yield << "+-" << omega_0_error << std::endl;
    std::cout << "0. Bin Phi Integral: " << phi_0_yield << "+-" << phi_0_error << std::endl;

    std::cout << "omega: " << "  " << omega_yield->GetBinContent(1) << "  " <<  omega_yield->GetBinError(1) << std::endl;
    std::cout << "phi: "   << "  " << phi_yield->GetBinContent(1) << "  " <<  phi_yield->GetBinError(1) << std::endl; 

    

    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,1,kTRUE)->GetDatahist()->Fit(pol3_1,"FM+","",0.6,1.0);
    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,1,kTRUE)->GetDatahist()->Fit(pol3_2,"FM+","",0.9,1.5);

    TH1D* ULS_omega_1 = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,1,kTRUE)->GetDatahist()->Clone("ULS_omega_1");
    TH1D* ULS_phi_1   = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,1,kTRUE)->GetDatahist()->Clone("ULS_phi_1");

    ULS_omega_1->Add(pol3_1,-1.);
    ULS_phi_1  ->Add(pol3_2,-1.);

    TH1D* NewSubSig_1 = hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kSubSig,1,kTRUE)->GetDatahist();

    for(Int_t i = 1; i<ULS_omega_1->GetNbinsX(); i++){
      if(NewSubSig_1->GetXaxis()->GetBinCenter(i) <= 0.9){	    
        NewSubSig_1->SetBinContent(i,ULS_omega_1->GetBinContent(i));
        NewSubSig_1->SetBinError(i,  ULS_omega_1->GetBinError(i));
      }
      else {
        NewSubSig_1->SetBinContent(i,ULS_phi_1->GetBinContent(i));
        NewSubSig_1->SetBinError(i,  ULS_phi_1->GetBinError(i));
      }
    }

    TF1* omega_fit1 = new TF1("omega_fit1", "pol1(0)+crystalball(2)",0.4,2.0);
    omega_fit1->SetLineColor(kOrange);
    //omega_fit1->SetParLimits(0,0,5.00e-04);
    omega_fit1->SetParLimits(5,0,1.);
    omega_fit1->SetParLimits(6,0,9999);

    omega_fit1->SetParameter(0,5.0e-04);
    omega_fit1->SetParameter(1,-3.23243e-04);

    omega_fit1->SetParameter(2,0.001);
    omega_fit1->SetParameter(3,0.78);
    omega_fit1->SetParameter(4,0.01);
    omega_fit1->SetParameter(5,0.1);
    omega_fit1->SetParameter(6,0.5);
    omega_fit1->SetNpx(1000);

    TF1* phi_fit1 = new TF1("phi_fit1", "pol1(0)+crystalball(2)",0.90,1.5);
    phi_fit1->SetParLimits(0,1.3e-03,2.00e-03);
    phi_fit1->SetParLimits(5,0,1.);
    phi_fit1->SetParLimits(6,0,9999);
    phi_fit1->SetParameter(0,6.0e-04);
    phi_fit1->SetParameter(1,-3.23243e-04);
    phi_fit1->SetNpx(1000);


    phi_fit1->SetParameter(2,0.0001);
    phi_fit1->SetParameter(3,1.02);
    phi_fit1->SetParameter(4,0.01);
    phi_fit1->SetParameter(5,0.01);
    phi_fit1->SetParameter(6,0.5);

    NewSubSig_1->Fit(omega_fit1,"MR+","",0.6,1.2);
    NewSubSig_1->Fit(phi_fit1,"MR+","",0.85,1.3);

    TH1D* Sig_1 = (TH1D*)NewSubSig_1->Clone("Sig1");
    TF1* sig_1_bkg = new TF1("sig_1_bkg", "pol1", 0.4,2.0);
    Double_t par1[2];
    omega_fit1->GetParameters(&par1[0]);
    sig_1_bkg->SetParameters(par1);
    Sig_1->Add(sig_1_bkg,-1.);
    Double_t omega_1_yield, omega_1_error;
    Double_t phi_1_yield, phi_1_error;

    omega_1_yield = Sig_1->IntegralAndError(Sig_1->GetXaxis()->FindBin(0.7), Sig_1->GetXaxis()->FindBin(0.85), omega_1_error);
    phi_1_yield = Sig_1->IntegralAndError(Sig_1->GetXaxis()->FindBin(0.95), Sig_1->GetXaxis()->FindBin(1.05), phi_1_error);

    omega_yield->SetBinContent(2,omega_1_yield);
    omega_yield->SetBinError(2,omega_1_error);

    phi_yield->SetBinContent(2,phi_1_yield);
    phi_yield->SetBinError(2,phi_1_error);

    Double_t SoverB_omega_1 = omega_1_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,1,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.6), Sig_0->GetXaxis()->FindBin(0.85));
    Double_t SoverB_phi_1   = phi_1_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,1,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.85), Sig_0->GetXaxis()->FindBin(1.1));

    std::cout << "S/B: omega: " << SoverB_omega_1 << "  phi: " << SoverB_phi_1 << std::endl;


    std::cout << "1. Bin Omega Integral: " << omega_1_yield << "+-" << omega_1_error << std::endl;
    std::cout << "1. Bin Phi Integral: " << phi_1_yield << "+-" << phi_1_error << std::endl;

    std::cout << "omega: "  << "  " << omega_yield->GetBinContent(2) << "  " <<  omega_yield->GetBinError(2) << std::endl;
    std::cout << "phi: "  << "  " << phi_yield->GetBinContent(2) << "  " <<  phi_yield->GetBinError(2) << std::endl; 



    

    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,2,kTRUE)->GetDatahist()->Fit(pol4_1,"FM+","",0.2,0.8);
    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,2,kTRUE)->GetDatahist()->Fit(pol8_2,"FM+","",0.75,1.4);

    TH1D* ULS_omega_2 = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,2,kTRUE)->GetDatahist()->Clone("ULS_omega_2");
    TH1D* ULS_phi_2   = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,2,kTRUE)->GetDatahist()->Clone("ULS_phi_2");

    ULS_omega_2->Add(pol4_1,-1.);
    ULS_phi_2  ->Add(pol8_2,-1.);

    TH1D* NewSubSig_2 = hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kSubSig,2,kTRUE)->GetDatahist();

    for(Int_t i = 1; i<ULS_omega_2->GetNbinsX(); i++){
      if(NewSubSig_2->GetXaxis()->GetBinCenter(i) <= 0.8){	    
        NewSubSig_2->SetBinContent(i,ULS_omega_2->GetBinContent(i));
        NewSubSig_2->SetBinError(i,  ULS_omega_2->GetBinError(i));
      }
      else {
        NewSubSig_2->SetBinContent(i,ULS_phi_2->GetBinContent(i));
        NewSubSig_2->SetBinError(i,  ULS_phi_2->GetBinError(i));
      }
    }

    TF1* omega_fit2 = new TF1("omega_fit2", "pol1(0)+crystalball(2)",0.4,2.0);
    omega_fit2->SetLineColor(kOrange);
    omega_fit2->SetParLimits(0,0,4.00e-04);
    omega_fit2->SetParLimits(4,0.01,1.);
    omega_fit2->SetParLimits(5,0.5,1.);
    omega_fit2->SetParLimits(6,0.001,9999);

    omega_fit2->SetParameter(0,3.0e-04);
    omega_fit2->FixParameter(1,-2.923243e-04);
   
    omega_fit2->SetParameter(2,0.001);
    omega_fit2->SetParameter(3,0.78);
    omega_fit2->SetParameter(4,0.01);
    omega_fit2->SetParameter(5,0.5);
    omega_fit2->SetParameter(6,0.5);
    omega_fit2->SetNpx(1000);

    TF1* phi_fit2 = new TF1("phi_fit2", "pol1(0)+crystalball(2)",0.88,1.4);
    phi_fit2->SetParLimits(0,3.00e-04,4.00e-04);
    phi_fit2->SetParLimits(1,-1,-2.95774e-04);
    phi_fit2->SetParLimits(3,1.0,1.1);

    phi_fit2->SetParLimits(5,0,1.);
    phi_fit2->SetParLimits(6,0,9999);
    phi_fit2->SetParameter(0,6.0e-04);
    phi_fit2->SetParameter(1,-3.23243e-04);
    phi_fit2->SetNpx(1000);


    phi_fit2->SetParameter(2,0.0001);
    phi_fit2->SetParameter(3,1.05);
    phi_fit2->SetParameter(4,0.01);
    phi_fit2->SetParameter(5,0.01);
    phi_fit2->SetParameter(6,0.5);

    NewSubSig_2->Fit(omega_fit2,"MR+","",0.5,1.3);
    NewSubSig_2->Fit(phi_fit2,"MR+","",0.85,1.3);

    TH1D* Sig_2 = (TH1D*)NewSubSig_2->Clone("Sig2");
    TF1* sig_2_bkg = new TF1("sig_2_bkg", "pol1", 0.4,2.0);
    Double_t par2[2];
    omega_fit2->GetParameters(&par2[0]);
    sig_2_bkg->SetParameters(par2);
    Sig_2->Add(sig_2_bkg,-1.);
    Double_t omega_2_yield, omega_2_error;
    Double_t phi_2_yield, phi_2_error;

    omega_2_yield = Sig_2->IntegralAndError(Sig_2->GetXaxis()->FindBin(0.7), Sig_2->GetXaxis()->FindBin(0.8), omega_2_error);
    phi_2_yield = Sig_2->IntegralAndError(Sig_2->GetXaxis()->FindBin(0.9), Sig_2->GetXaxis()->FindBin(1.1), phi_2_error);

    omega_yield->SetBinContent(3,omega_2_yield);
    omega_yield->SetBinError(3,omega_2_error);

    phi_yield->SetBinContent(3,phi_2_yield);
    phi_yield->SetBinError(3,phi_2_error);

    Double_t SoverB_omega_2 = omega_2_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,2,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.6), Sig_0->GetXaxis()->FindBin(0.85));
    Double_t SoverB_phi_2   = phi_2_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,2,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.85), Sig_0->GetXaxis()->FindBin(1.1));

    std::cout << "S/B: omega: " << SoverB_omega_2 << "  phi: " << SoverB_phi_2 << std::endl;


    std::cout << "2. Bin Omega Integral: " << omega_2_yield << "+-" << omega_2_error << std::endl;
    std::cout << "2. Bin Phi Integral: " << phi_2_yield << "+-" << phi_2_error << std::endl;

    std::cout << "omega: " << "  " << omega_yield->GetBinContent(3) << "  " <<  omega_yield->GetBinError(3) << std::endl;
    std::cout << "phi: " << "  " << phi_yield->GetBinContent(3) << "  " <<  phi_yield->GetBinError(3) << std::endl; 



    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,3,kTRUE)->GetDatahist()->Fit(pol2_1,"FM+","",0.2,0.9);
    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,3,kTRUE)->GetDatahist()->Fit(pol3_2,"FM+","",0.9,1.5);


    TH1D* ULS_omega_3 = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,3,kTRUE)->GetDatahist()->Clone("ULS_1");
    TH1D* ULS_phi_3   = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,3,kTRUE)->GetDatahist()->Clone("ULS_2");

    ULS_omega_3->Add(pol2_1,-1.);
    ULS_phi_3  ->Add(pol3_2,-1.);

    TH1D* NewSubSig_3 = hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kSubSig,3,kTRUE)->GetDatahist();

    for(Int_t i = 1; i<ULS_omega_3->GetNbinsX(); i++){
      if(NewSubSig_3->GetXaxis()->GetBinCenter(i) <= 0.9){	    
        NewSubSig_3->SetBinContent(i,ULS_omega_3->GetBinContent(i));
        NewSubSig_3->SetBinError(i,  ULS_omega_3->GetBinError(i));
      }
      else {
        NewSubSig_3->SetBinContent(i,ULS_phi_3->GetBinContent(i));
        NewSubSig_3->SetBinError(i,  ULS_phi_3->GetBinError(i));
      }
    }

    TF1* omega_fit3 = new TF1("omega_fit3", "pol1(0)+crystalball(2)",0.4,2.0);
    omega_fit3->SetLineColor(kOrange);
    omega_fit3->SetParLimits(0,0,5.00e-04);
    omega_fit3->SetParLimits(5,0,1.);
    omega_fit3->SetParLimits(6,0,9999);

    omega_fit3->SetParameter(0,5.0e-04);
    omega_fit3->SetParameter(1,-3.23243e-04);

    omega_fit3->SetParameter(2,0.001);
    omega_fit3->SetParameter(3,0.78);
    omega_fit3->SetParameter(4,0.01);
    omega_fit3->SetParameter(5,0.1);
    omega_fit3->SetParameter(6,0.5);
    omega_fit3->SetNpx(1000);

    TF1* phi_fit3 = new TF1("phi_fit3", "pol1(0)+crystalball(2)",0.88,1.5);
    phi_fit3->SetParLimits(0,0,6.00e-04);
    phi_fit3->SetParLimits(5,0,1.);
    phi_fit3->SetParLimits(6,0,9999);
    phi_fit3->SetParameter(0,6.0e-04);
    phi_fit3->SetParameter(1,-3.23243e-04);
    phi_fit3->SetNpx(1000);


    phi_fit3->SetParameter(2,0.0001);
    phi_fit3->SetParameter(3,1.05);
    phi_fit3->SetParameter(4,0.01);
    phi_fit3->SetParameter(5,0.01);
    phi_fit3->SetParameter(6,0.5);

    NewSubSig_3->Fit(omega_fit3,"MR+","",0.4,1.6);
    NewSubSig_3->Fit(phi_fit3,"MR+","",0.88,1.5);

    TH1D* Sig_3 = (TH1D*)NewSubSig_3->Clone("Sig3");
    TF1* sig_3_bkg = new TF1("sig_3_bkg", "pol1", 0.4,2.0);
    Double_t par3[2];
    omega_fit3->GetParameters(&par3[0]);
    sig_3_bkg->SetParameters(par3);
    Sig_3->Add(sig_3_bkg,-1.);
    Double_t omega_3_yield, omega_3_error;
    Double_t phi_3_yield, phi_3_error;

    omega_3_yield = Sig_3->IntegralAndError(Sig_3->GetXaxis()->FindBin(0.6), Sig_3->GetXaxis()->FindBin(0.85), omega_3_error);
    phi_3_yield = Sig_3->IntegralAndError(Sig_3->GetXaxis()->FindBin(0.9), Sig_3->GetXaxis()->FindBin(1.1), phi_3_error);

    omega_yield->SetBinContent(4,omega_3_yield);
    omega_yield->SetBinError(4,omega_3_error);

    phi_yield->SetBinContent(4,phi_3_yield);
    phi_yield->SetBinError(4,phi_3_error);

    Double_t SoverB_omega_3 = omega_3_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,3,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.6), Sig_0->GetXaxis()->FindBin(0.85));
    Double_t SoverB_phi_3   = phi_3_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,3,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.85), Sig_0->GetXaxis()->FindBin(1.1));

    std::cout << "S/B: omega: " << SoverB_omega_3 << "  phi: " << SoverB_phi_3 << std::endl;


    std::cout << "3. Bin Omega Integral: " << omega_3_yield << "+-" << omega_3_error << std::endl;
    std::cout << "3. Bin Phi Integral: " << phi_3_yield << "+-" << phi_3_error << std::endl;

    std::cout << "omega: "  << "  " << omega_yield->GetBinContent(4) << "  " <<  omega_yield->GetBinError(4) << std::endl;
    std::cout << "phi: " << "  " << phi_yield->GetBinContent(4) << "  " <<  phi_yield->GetBinError(4) << std::endl; 



    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,4,kTRUE)->GetDatahist()->Fit(pol1_1,"FM+","",0.3,1.2);
    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,4,kTRUE)->GetDatahist()->Fit(pol3_2,"FM+","",0.3,1.5);

    TH1D* ULS_omega_4 = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,4,kTRUE)->GetDatahist()->Clone("ULS_omega_4");
    TH1D* ULS_phi_4   = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,4,kTRUE)->GetDatahist()->Clone("ULS_phi_4");

    ULS_omega_4->Add(pol1_1,-1.);
    ULS_phi_4  ->Add(pol3_2,-1.);

    TH1D* NewSubSig_4 = hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kSubSig,4,kTRUE)->GetDatahist();

    for(Int_t i = 1; i<ULS_omega_4->GetNbinsX(); i++){
      if(NewSubSig_4->GetXaxis()->GetBinCenter(i) <= 1.2){	    
        NewSubSig_4->SetBinContent(i,ULS_omega_4->GetBinContent(i));
        NewSubSig_4->SetBinError(i,  ULS_omega_4->GetBinError(i));
      }
      else {
        NewSubSig_4->SetBinContent(i,ULS_phi_4->GetBinContent(i));
        NewSubSig_4->SetBinError(i,  ULS_phi_4->GetBinError(i));
      }
    }

    TF1* omega_fit4 = new TF1("omega_fit4", "pol1(0)+crystalball(2)",0.4,2.0);
    omega_fit4->SetLineColor(kOrange);
    omega_fit4->SetParLimits(0,0,1.250e-04);
    omega_fit4->SetParLimits(5,0,1.);
    omega_fit4->SetParLimits(6,0,9999);

    omega_fit4->SetParameter(0,5.0e-04);
    omega_fit4->SetParameter(1,-3.23243e-04);

    omega_fit4->SetParameter(2,0.001);
    omega_fit4->SetParameter(3,0.78);
    omega_fit4->SetParameter(4,0.01);
    omega_fit4->SetParameter(5,0.1);
    omega_fit4->SetParameter(6,0.5);
    omega_fit4->SetNpx(1000);

    TF1* phi_fit4 = new TF1("phi_fit4", "pol1(0)+crystalball(2)",0.88,1.5);
    phi_fit4->SetParLimits(0,0,1.300e-04);
    phi_fit4->SetParLimits(5,0,1.);
    phi_fit4->SetParLimits(6,0,9999);
    phi_fit4->SetParameter(0,6.0e-04);
    phi_fit4->SetParameter(1,-3.23243e-04);
    phi_fit4->SetNpx(1000);


    phi_fit4->SetParameter(2,0.0001);
    phi_fit4->SetParameter(3,1.05);
    phi_fit4->SetParameter(4,0.01);
    phi_fit4->SetParameter(5,0.01);
    phi_fit4->SetParameter(6,0.5);

    NewSubSig_4->Fit(omega_fit4,"MR+","",0.4,1.4);
    NewSubSig_4->Fit(phi_fit4,"MR+","",0.88,1.4);

    TH1D* Sig_4 = (TH1D*)NewSubSig_4->Clone("Sig4");
    TF1* sig_4_bkg = new TF1("sig_4_bkg", "pol1", 0.4,2.0);
    Double_t par4[2];
    omega_fit4->GetParameters(&par4[0]);
    sig_4_bkg->SetParameters(par4);
    Sig_4->Add(sig_4_bkg,-1.);
    Double_t omega_4_yield, omega_4_error;
    Double_t phi_4_yield, phi_4_error;

    omega_4_yield = Sig_4->IntegralAndError(Sig_4->GetXaxis()->FindBin(0.6), Sig_4->GetXaxis()->FindBin(0.9), omega_4_error);
    phi_4_yield = Sig_4->IntegralAndError(Sig_4->GetXaxis()->FindBin(0.9), Sig_4->GetXaxis()->FindBin(1.1), phi_4_error);

    omega_yield->SetBinContent(5,omega_4_yield);
    omega_yield->SetBinError(5,omega_4_error);

    phi_yield->SetBinContent(5,phi_4_yield);
    phi_yield->SetBinError(5,phi_4_error);

    Double_t SoverB_omega_4 = omega_4_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,4,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.6), Sig_0->GetXaxis()->FindBin(0.9));
    Double_t SoverB_phi_4   = phi_4_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,4,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.9), Sig_0->GetXaxis()->FindBin(1.1));

    std::cout << "S/B: omega: " << SoverB_omega_4 << "  phi: " << SoverB_phi_4 << std::endl;


    std::cout << "4. Bin Omega Integral: " << omega_4_yield << "+-" << omega_4_error << std::endl;
    std::cout << "4. Bin Phi Integral: " << phi_4_yield << "+-" << phi_4_error << std::endl;

    std::cout << "omega: "  << "  " << omega_yield->GetBinContent(5) << "  " <<  omega_yield->GetBinError(5) << std::endl;
    std::cout << "phi: "  << "  " << phi_yield->GetBinContent(5) << "  " <<  phi_yield->GetBinError(5) << std::endl; 



    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,5,kTRUE)->GetDatahist()->Fit(pol3_1,"FM+","",0.6,1.0);
    hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kBkg,5,kTRUE)->GetDatahist()->Fit(pol3_2,"FM+","",0.8,1.5);

    TH1D* ULS_omega_5 = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,5,kTRUE)->GetDatahist()->Clone("ULS_omega_5");
    TH1D* ULS_phi_5   = (TH1D*)hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,5,kTRUE)->GetDatahist()->Clone("ULS_phi_5");

    ULS_omega_5->Add(pol3_1,-1.);
    ULS_phi_5  ->Add(pol3_2,-1.);

    TH1D* NewSubSig_5 = hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kSubSig,5,kTRUE)->GetDatahist();

    for(Int_t i = 1; i<ULS_omega_5->GetNbinsX(); i++){
      if(NewSubSig_5->GetXaxis()->GetBinCenter(i) <= 0.9){	    
        NewSubSig_5->SetBinContent(i,ULS_omega_5->GetBinContent(i));
        NewSubSig_5->SetBinError(i,  ULS_omega_5->GetBinError(i));
      }
      else {
        NewSubSig_5->SetBinContent(i,ULS_phi_5->GetBinContent(i));
        NewSubSig_5->SetBinError(i,  ULS_phi_5->GetBinError(i));
      }
    }

    TF1* omega_fit5 = new TF1("omega_fit5", "pol1(0)+crystalball(2)",0.4,2.0);
    omega_fit5->SetLineColor(kOrange);
    omega_fit5->SetParLimits(0,0,5.00e-04);
    omega_fit5->SetParLimits(5,0,1.);
    omega_fit5->SetParLimits(6,0,9999);

    omega_fit5->SetParameter(0,5.0e-04);
    omega_fit5->SetParameter(1,-3.23243e-04);

    omega_fit5->SetParameter(2,0.001);
    omega_fit5->SetParameter(3,0.78);
    omega_fit5->SetParameter(4,0.01);
    omega_fit5->SetParameter(5,0.1);
    omega_fit5->SetParameter(6,0.5);
    omega_fit5->SetNpx(1000);

    TF1* phi_fit5 = new TF1("phi_fit5", "pol1(0)+crystalball(2)",0.88,1.5);
    phi_fit5->SetParLimits(0,0,6.00e-04);
    phi_fit5->SetParLimits(5,0,1.);
    phi_fit5->SetParLimits(6,0,9999);
    phi_fit5->SetParameter(0,6.0e-04);
    phi_fit5->SetParameter(1,-3.23243e-04);
    phi_fit5->SetNpx(1000);


    phi_fit5->SetParameter(2,0.0001);
    phi_fit5->SetParameter(3,1.05);
    phi_fit5->SetParameter(4,0.01);
    phi_fit5->SetParameter(5,0.01);
    phi_fit5->SetParameter(6,0.5);

    NewSubSig_5->Fit(omega_fit5,"MR+","",0.4,1.6);
    NewSubSig_5->Fit(phi_fit5,"MR+","",0.88,1.5);

    TH1D* Sig_5 = (TH1D*)NewSubSig_5->Clone("Sig5");
    TF1* sig_5_bkg = new TF1("sig_5_bkg", "pol1", 0.4,2.0);
    Double_t par5[2];
    omega_fit5->GetParameters(&par5[0]);
    sig_5_bkg->SetParameters(par5);
    Sig_5->Add(sig_5_bkg,-1.);
    Double_t omega_5_yield, omega_5_error;
    Double_t phi_5_yield, phi_5_error;

    omega_5_yield = Sig_5->IntegralAndError(Sig_5->GetXaxis()->FindBin(0.6), Sig_5->GetXaxis()->FindBin(0.85), omega_5_error);
    phi_5_yield = Sig_5->IntegralAndError(Sig_5->GetXaxis()->FindBin(0.85), Sig_5->GetXaxis()->FindBin(1.1), phi_5_error);


    Double_t SoverB_omega_5 = omega_5_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,5,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.6), Sig_0->GetXaxis()->FindBin(0.85));
    Double_t SoverB_phi_5   = phi_5_yield-hanCompare->GetLmSignalI(0)->GetLmHist(LmSignal::kUnlike,5,kTRUE)->GetDatahist()-> Integral(Sig_0->GetXaxis()->FindBin(0.85), Sig_0->GetXaxis()->FindBin(1.1));

    std::cout << "S/B: omega: " << SoverB_omega_5 << "  phi: " << SoverB_phi_5 << std::endl;



    std::cout << "5.te Bin Omega Integral: " << omega_5_yield << "+-" << omega_5_error << std::endl;
    std::cout << "5.te Bin Phi Integral: " << phi_5_yield << "+-" << phi_5_error << std::endl;

    for(int i = 1; i < n_ptee_slices+1; i++){
    
      std::cout << "omega: " << i << "  " << omega_yield->GetBinContent(i) << "  " <<  omega_yield->GetBinError(i) << std::endl;
      std::cout << "phi: " << i << "  " << phi_yield->GetBinContent(i) << "  " <<  phi_yield->GetBinError(i) << std::endl; 

    } 

    const Char_t* filename_yield = "YieldExtraction.root";
    TFile* _fileOut2 = new TFile(filename_yield, "RECREATE");
    omega_yield->Write();
    phi_yield->Write();
    _fileOut2->Close();
    ...
    if(kDoMassSlices == kFALSE){
      for (unsigned int i = 0; i < binsPtee_forMee.size(); ++i){
        if(kDoNonLogY){ 
          ...
          if(i==0) hanCompare->SetPlotRangeY(0., 0.02); 
          if(i==1) hanCompare->SetPlotRangeY(0., 0.03); 
      ...
      for (unsigned int i = 0; i < binsMee_forPtee.size(); ++i){
        ...
        if(kDoNonLogY){ 
          ...
          if(i==2) hanCompare->SetPlotRangeY(0.001, 0.04);
...
//      cocktmanReso->SetInputfilesAndHistsHigh(const_cast<char*>(cocktail_files[2].Data()), const_cast<char*>(cocktail_histograms[1].Data())); // 2Di
    ...
    if(kDoLFsum) cocktmanReso->Process(LmCocktailMan::kLFsum);
    else cocktmanReso->Process(LmCocktailMan::kResonances);
    ...
    if(kDoSUMonly) cockt00->SetDrawOption(LmCocktail::kOnlySum);
    else if(kDoSUMnot){ 
...
//        if(i==1) hanCompare->SetPlotRangeY(0.00001, 0.45); 
        if(i==1) hanCompare->SetPlotRangeY(0.00001, 0.45); 
        if(i==2) hanCompare->SetPlotRangeY(0.0000001, 0.0005);
